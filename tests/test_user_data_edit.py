...
class TestUserDataEdit:
    @allure.title('Тест изменения данных авторизированного пользователя')
    ...
    @allure.title('Тест попытки изменения emaila на уже используемый')
    ...
    @allure.title('Тест изменения данных не авторизированного пользователя')
    @pytest.mark.parametrize('test_data', ['email', 'name'])
